...
type RestOf<T extends IPortaria | NewPortaria> = Omit<T, 'data' | 'dataInicio' | 'dataFim' | 'createdAt' | 'updatedAt' | 'deletedAt'> & {
  data?: string | null;
...
export class PortariaService {
  ...
  protected convertDateFromClient<T extends IPortaria | NewPortaria | PartialUpdatePortaria>(portaria: T): RestOf<T> {
    return {
      ...
      dataInicio: portaria.dataInicio?.format(DATE_FORMAT) ?? null,
      dataFim: portaria.dataFim?.format(DATE_FORMAT) ?? null,
  ...
  protected convertDateFromServer(restPortaria: RestPortaria): IPortaria {
    return {
      ...
      dataInicio: restPortaria.dataInicio ? dayjs(restPortaria.dataInicio) : undefined,
      dataFim: restPortaria.dataFim ? dayjs(restPortaria.dataFim) : undefined,
